/**
 * @file encode_scaler_substream.c
 *
 * Support platform: GM8210, GM8287, GM8139
 * This source code is encoder main & substream by scaler operation
 * Please note, GM8210 capture can provide 4 paths per channel. To use substream, plesae
 * refer to encode_capture_substream.c.
 * (此示例为主码流和子码流现时编码程序)
 * Copyright (C) 2012 GM Corp. (http://www.grain-media.com)
 *
 * $Revision: 1.33 $
 * $Date: 2014/05/14 06:28:44 $
 *
 */
/**
 * @example encode_scaler_substream.c
 */
...
static void *encode_thread(void *arg)
{
    ...
    sprintf(filename, "ch%d_%dx%d_scaler_main.264", ch, gm_system.cap[ch].dim.width,
    ...
    sprintf(filename, "ch%d_%dx%d_scaler_sub.264", ch, gm_system.cap[ch].dim.width / 2,
...
int main(void)
{
    ...
    /* 
        Please note, GM8210 should not use this sample to do substream. You should bind
        another capture path to do substream (refer to encode_capture_substream.c)
     */
    ...
    main_bindfd = gm_bind(groupfd, capture_object, main_enc_object);

    /*** Init scaler substream */
    h264e_attr.ratectl.bitrate = 128;  // 128Kbps
    h264e_attr.frame_info.framerate = 10;
    h264e_attr.dim.width = 320;
    h264e_attr.dim.height = 240;

    gm_set_attr(sub_enc_object, &h264e_attr);
    /*** bind sub stream recording (绑定捕捉对象到编码对象) */
    sub_bindfd = gm_bind(groupfd, capture_object, sub_enc_object);
