/**
 * @file encode_with_deinterlace.c
 *
 * Support platform: GM8210, GM8287, GM8139
 * This source code is encoder with 3DI sample
 * (此示例为开3DI编码程序)
 ...
 */
/**
 * @example encode_with_deinterlace.c
 */
...
static void *encode_thread(void *arg)
{
    ...
    sprintf(filename, "ch%d_%dx%d_deinterlaced.264", ch, gm_system.cap[ch].dim.width,
...
int main(void)
{
    ...
    cap_3di_attr.deinterlace = 1; // 1 enable (开启), 2 disable(关闭)
    cap_3di_attr.denoise = 1; // 1 enable (开启), 2 disable(关闭)
    gm_set_attr(capture_object, &cap_3di_attr); //set 3DI attribute(设置3DI属性)
