...
static void *encode_thread(void *arg)
{
    ...
    sprintf(filename, "ch%d_%dx%d_osd.264", ch, gm_system.cap[ch].dim.width, gm_system.cap[ch].dim.height);
...
void update_osd_font_with_gm_chinese_title(void)
{
    ...
    for (j = 0; j <= 0xf; j++)
    {
        ...
        {
            new_font.bitmap[i] = (osd_gm_title[j].fbitmap[i * 2 + 1] << 8) | osd_gm_title[j].fbitmap[i * 2];
        }
